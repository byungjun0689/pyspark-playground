...
if [ ! -n "${ES_LOCAL_LICENSE:-}" ] && [ "$today" -gt 1735111994 ]; then
  echo "---------------------------------------------------------------------"
  ...
  docker-compose -f docker-compose.shared.yml up --wait elasticsearch >/dev/null 2>&1
...
docker-compose -f docker-compose.shared.yml up --wait
